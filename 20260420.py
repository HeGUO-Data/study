...
# 生成器表达式
[x * x for x in range(10)] # 列表生成器，返回一个列表对象
(x * x for x in range(10)) # 生成器表达式，返回一个生成器对象，生成器对象是一个迭代器，可以使用next()函数获取下一个元素，也可以使用for循环遍历生成器对象
{x: x * x for x in range(10)} # 字典生成器，返回一个字典对象
{x for x in range(10)} # 集合生成器，返回一个集合对象


# 模块
if __name__ == "__main__":  
# __name__是一个特殊变量，表示当前模块的名字，如果当前模块是主模块，那么__name__的值就是"__main__"，否则就是模块的名字
    print("这是主模块")

# 搜索路径
import sys
print(sys.path) 
# sys.path是一个列表，包含了Python解释器搜索模块的路径，默认情况下包含当前目录、标准库目录和第三方库目录，
# 可以通过修改sys.path来添加新的搜索路径
sys.path.append("/path/to/your/module") # 添加新的搜索路径

# 包
# 包是一个包含__init__.py文件的目录，__init__.py文件可以是空的，也可以包含包的初始化代码，
# 包可以包含多个模块和子包，包的名字就是目录的名字，模块的名字就是文件的名字，子包的名字就是子目录的名字。
# 导入包中的模块
